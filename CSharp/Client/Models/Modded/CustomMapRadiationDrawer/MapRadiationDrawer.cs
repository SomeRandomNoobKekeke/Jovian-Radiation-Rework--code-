...
{
  public partial class CustomMapRadiationDrawerModel
  {
    /// <summary>
    /// Note that this is not only draws radiaton, it also calculates radiationMultiplier
    /// mb i should split it into 2 models
    /// </summary>
    public class CustomMapRadiationDrawer : IMapRadiationDrawer
    {
      ...
      public void Draw(Radiation _, SpriteBatch spriteBatch, Rectangle container, float zoom)
      {
        ...
        radiationMainSprite?.Sprite.DrawTiled(spriteBatch, topLeft, size, color: Settings.AreaColor, startOffset: Vector2.Zero, textureScale: spriteScale);
        ...
        if (_.radiationEdgeAnimSheet != null)
        {
          for (float i = 0; i <= size.Y; i += _.radiationEdgeAnimSheet.FrameSize.Y / 2f * zoom)
          {
            ...
            _.radiationEdgeAnimSheet.Draw(spriteBatch, sprite, topRight + new Vector2(0, i), Settings.BorderColor, origin, 0f, spriteScale);
          }
        }
        ...
        if (container.Contains(PlayerInput.MousePosition))
        {
          ...
          {
            _.radiationMultiplier = (int)distanceFromRight;
          }
        }
      }
    }
